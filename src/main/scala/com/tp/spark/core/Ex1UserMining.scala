...
object Ex1UserMining {
  ...
  def tweetByUserNumber(): RDD[(String, Int)] = {
    // loadData().groupBy(_.user).mapValues(_.size)
  ...
  def topTenTwitterers(): Array[(String, Int)] = {
    loadData().map(tweet => (tweet.user, 1)).reduceByKey(_ + _).takeOrdered(10)(Ordering[Int].reverse.on(_._2))
    // loadData().groupBy(_.user).mapValues(_.size).takeOrdered(10)(Ordering[Int].reverse.on(_._2))
  }
