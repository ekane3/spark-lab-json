...
object Ex4InvertedIndex {
  ...
  def invertedIndex(): Map[String, Iterable[Tweet]] = {
    ...
    //val invertedIndex = tweets.flatMap(tweet => tweet.text.split(" ").map(word => (word, tweet))).groupByKey().collectAsMap()
    val invertedIndex = tweets.flatMap(tweet => tweet.text.split(" ")
     ...
     /*
    val invertedIndex = tweets.flatMap (tweet => tweet.hashtags.map (hashtag => (hashtag, tweet)))
        .groupByKey()
        .mapValues (list => list.toList.sortBy (tweet => tweet.timestamp))
        */
    invertedIndex
  }
