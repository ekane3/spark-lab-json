...
object Ex0Wordcount {
  ...
  def filterOnWordcount(): RDD[(String, Int)] = {
    loadData.map(word => (word,1) ) // we attribute 1 to each word
            .reduceByKey(_+_) // we count the amount of each word
            .filter(_._2 > 4) // we keep only the word which appear more than 4 times
  }
